...
function create_bc(dh)
    ...
    Ferrite.add!(
    dbc,
        Dirichlet(:u, getfaceset(dh.grid, "left"), (x, t) -> zero(Vec{2}), [1,2]),
    )
    Ferrite.add!(
        dbc,
        Dirichlet(:u, getfaceset(dh.grid, "top"), (x, t) -> zero(Vec{1}), [2]),
        )
    Ferrite.add!(
        dbc,
        Dirichlet(:u, getfaceset(dh.grid, "bottom"), (x, t) -> zero(Vec{1}), [2]),
        )
    close!(dbc)
    t = 0.0
    update!(dbc, t)
...
x_points = [Vec((x, Lⱼₛ/2)) for x in range(0, Lᵢₛ, length=101)];
ph = PointEvalHandler(grid, x_points);
u_points = Ferrite.get_point_values(ph, dh, u, :u);
# extract in x direction
xᵥ = getindex.(x_points, 1)
uxᵥ_num = getindex.(u_points, 1)
C = p * ((1 - ν - 2 *(ν^2)) / (1 - ν))
uₗ = xᵥ * C / Eᵣ
